...
int main()
{
	...
	*/
	...
	// This will wait 3 Secounds (3000) Millisecounds until ...
	Sleep(3000);
	// System will CLS (Clear The Console)
	system("CLS");

	int option; // This was the option which allowed you to choose between Login, Register, Upgrade and License key only. You wont need it anymore
	std::string username; // This is the string for the username input, this KeyAuth version is License key only so you dont need it anymore
	std::string password; // This is the string for the password input, this KeyAuth version is License key only so you dont need it anymore
	std::string key; // This is the string for the Key, you will need it.

	// Now it will ask you for the license key
	std::cout << skCrypt("\n Enter license: ");
	std::cin >> key;

	KeyAuthApp.license(key); /// Checks if key is valid/invalid
	...
	*/
	...
	// This is the end code. If you want to add this to your programm, just remove the next 3 Lines of code and replace it with yours code which will run after the user logged in
	std::cout << skCrypt("\n\n Closing in ten seconds...");
	// Wait 10 secounds (10000 millisecounds)
	Sleep(10000);
